...
import (
	...
	"github.com/smart-kf/goim2/pkg/bytes"
	xtime "github.com/smart-kf/goim2/pkg/time"
	"github.com/smart-kf/goim2/pkg/websocket"
	"github.com/smart-kf/goim2/services/comet/conf"
)
...
func (s *Server) ServeWebsocket(conn net.Conn, rp, wp *bytes.Pool, tr *xtime.Timer) {
	...
	if err != nil || req.RequestPath() != "/sub" {
		conn.Close()
		tr.Del(trd)
		rp.Put(rb)
		if err != io.EOF {
...
func (s *Server) authWebsocket(ctx context.Context, ws *websocket.Conn, p *protocol.Proto, authInfo AuthInfo) (mid int64, key, rid string, accepts []int32, hb time.Duration, err error) {
	for {
		if err = p.ReadWebsocket(ws); err != nil {
			return
		}
	...
	if mid, key, rid, accepts, hb, err = s.Connect(ctx, p, authInfo); err != nil {
		return
	}
